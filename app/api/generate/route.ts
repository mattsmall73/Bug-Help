...
export async function POST(req: NextRequest) {
  ...
  if (!pastedText.trim() && files.length === 0) {
    return NextResponse.json({ error: "Nothing to work with. Paste text or attach a file." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!html) {
      return NextResponse.json(
        { error: "Claude returned a response that wasn't valid HTML. Try again." },
        { status: 502 }
      );
    }
